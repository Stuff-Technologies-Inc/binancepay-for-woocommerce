...
class OrderStates {
	...
	public function getOrderStateLabels(): array {
		return [
			self::NEW => _x('New', 'global_settings', 'binancepay-for-woocommerce'),
			self::PROCESSING => _x('Paid', 'global_settings', 'binancepay-for-woocommerce'),
			self::SETTLED => _x('Settled', 'global_settings', 'binancepay-for-woocommerce'),
			self::SETTLED_PAID_OVER => _x('Settled (paid over)', 'global_settings', 'binancepay-for-woocommerce'),
			self::INVALID => _x('Invalid', 'global_settings', 'binancepay-for-woocommerce'),
			self::EXPIRED => _x('Expired', 'global_settings', 'binancepay-for-woocommerce'),
			self::EXPIRED_PAID_PARTIAL => _x('Expired with partial payment', 'global_settings', 'binancepay-for-woocommerce')
		];
	}

	public function renderOrderStatesHtml($value) {
		// Todo: mabye refactor to be done in a separate template file.
		$binancepayStates = $this->getOrderStateLabels();
		$defaultStates = $this->getDefaultOrderStateMappings();

		$wcStates = wc_get_order_statuses();
		$wcStates = [self::IGNORE => _x('- no mapping / defaults -', 'global_settings', 'binancepay-for-woocommerce')] + $wcStates;
		...
		<tr valign="top">
			...
			<td class="forminp" id="<?php echo esc_attr($value['id']) ?>">
				<table cellspacing="0">
					...
					foreach ($binancepayStates as $binancepayState => $binancepayName) {
						?>
						<tr>
							<th><?php echo esc_html($binancepayName); ?></th>
							<td>
								<select name="<?php echo esc_attr($value['id']) ?>[<?php echo esc_html($binancepayState); ?>]">
									<?php

									foreach ($wcStates as $wcState => $wcName) {
										$selectedOption = $orderStates[$binancepayState];

										if (true === empty($selectedOption)) {
											$selectedOption = $defaultStates[$binancepayState];
										}
